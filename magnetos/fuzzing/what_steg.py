...
# 集成外部 stegdetect-py（纯 Python 实现的 stegdetect）
# stegdetect-py 目录位于 magnetos 项目的上一级目录
_STEGDETECT_PY_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', '..', '..', 'stegdetect-py')
)
if os.path.isdir(_STEGDETECT_PY_PATH) and _STEGDETECT_PY_PATH not in sys.path:
    sys.path.insert(0, _STEGDETECT_PY_PATH)
...
一些依赖还需要手动安装
stegdetect 已替换为纯 Python 实现（stegdetect-py），无需外部安装
TODO:
FFD9 后的文件内容显示出来
"""
...
class WhatSteg(object):
    ...
    def stegdetect(self):
        """
        用于检测 jpg 的隐写，使用 stegdetect-py（纯 Python 实现）。
        :return:
        """
        if self.file_type == 'jpg':
            logger.info('\n--------------------')
            logger.info('run stegdetect (Python)')

            # 导入 stegdetect-py 的检测 API
            try:
                from stegdetect.detectors import detect, FLAG_ALL
            except ImportError:
                logger.warning('[*] stegdetect-py 不可用，跳过 jpg 隐写检测')
                logger.warning('[*] 请在 stegdetect-py 目录下运行: uv run stegdetect.py %s' % self.file_path)
                return

            scale = 5.0  # 灵敏度，与原 stegdetect -s 5 保持一致
            try:
                results = detect(self.file_path, flags=FLAG_ALL, scale=scale)
            except Exception as e:
                logger.error('[*] stegdetect 检测出错: %s' % e)
                return
            ...
            for r in results:
                logger.info('stegdetect result: type=%s, confidence=%.4f, bits=%d' % (
                    r.stego_type, r.confidence, r.estimated_bits))

                if r.stego_type == 'jsteg':
                    self.result_list.append('[*] 使用了 jsteg 隐写')
                elif r.stego_type == 'jphide':
                    ...
                    self.result_list.append(text)
                elif r.stego_type == 'outguess':
                    self.result_list.append('[*] 使用了 outguess 隐写')
                elif r.stego_type == 'f5':
                    self.result_list.append('[*] 使用了 F5 隐写')
                elif r.stego_type == 'invis':
                    self.result_list.append('[*] 使用了 invisible secrets 隐写')

            # 检测文件尾是否附加了数据（FFD9 之后）
            self._check_appended_data()

    def _check_appended_data(self):
        """
        检测 jpg 文件尾 FFD9 之后是否附加了额外数据。
        """
        try:
            with open(self.file_path, 'rb') as f:
                data = f.read()
        ...
        for root, dirs, files in os.walk(binwalk_path):
            for f_name in files:
                ...
                if file_ext == '':
                    ...
                    WhatFormat(path, out_path, exclude=['bmp', 'gif', 'pdf']).run()
    ...
    def what_format(self):
        ...
        self.remove_dir(out_path)
        WhatFormat(self.file_path, out_path, exclude=['bmp', 'gif', 'pdf']).run()
